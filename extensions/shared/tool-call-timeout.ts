...
export async function runWithToolCallTimeout<T>(toolName: string, timeoutMs: number, signal: AbortSignal | undefined, execute: (signal: AbortSignal) => Promise<T>) {
...
export function createToolCallTimeoutGuard(timeoutMs = CHILD_TOOL_CALL_TIMEOUT_MS) {
	...
	const wrap = (definition: ToolDefinition) => {
		...
		definition.execute = async (toolCallId, params, signal, onUpdate, ctx) =>
			runWithToolCallTimeout(definition.name, timeoutMs, signal, (signal) => execute.call(definition, toolCallId, params, signal, onUpdate, ctx));
	};
