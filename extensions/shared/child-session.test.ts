...
import { createAgentSession, DefaultResourceLoader, defineTool, ProjectTrustStore, SessionManager, SettingsManager, type SessionShutdownEvent } from '@earendil-works/pi-coding-agent';
...
async function withTempDir(run: (directory: string) => Promise<void>) {
	const directory = await mkdtemp(
		path.join(tmpdir(), 'pi-child-policy-'),
	);
...
test('child denylist keeps extension and workflow structured tools available', async () => {
	await withTempDir(async (directory) => {
		...
		assert.deepEqual([...CHILD_EXCLUDED_TOOL_NAMES], ['subagent_spawn', 'subagent_wait', 'subagent_cancel', 'subagent_check', 'subagent_list', 'workflow', 'ask_user']);
...
test('resource loading gates project extensions but retains global extensions', async () => {
    ...
    `;
		...
		const untrustedTools = untrusted.loader.getExtensions().extensions.flatMap((extension) => [...extension.tools.keys()]);
...
test('shutdown helper balances hooks and disposal despite errors', async () => {
	...
	await Promise.all([shutdownAndDisposeChildSession(session), shutdownAndDisposeChildSession(session), shutdownAndDisposeChildSession(session)]);
...
test('shutdown helper bounds a stuck hook before disposal', async () => {
	...
	await shutdownAndDisposeChildSession(
		session,
		{ timeoutMs: 10 },
	);
