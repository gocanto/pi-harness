import { countStates, formatElapsed, resultJson, shortenHome, type WorkflowDetails } from '@workflows/model.ts';
...
export const WORKFLOW_PARAMETER_DESCRIPTIONS = {
	script: 'JavaScript workflow script. May start with `export const meta = {...}`, then use phase(), agent(), parallel(), args, and a final `return`.',
	...
	background: 'Run in the background: the tool returns a run id immediately and you receive a follow-up message when the workflow finishes. Defaults to false (blocking with live progress).',
};
...
export const WORKFLOW_TOOL_DESCRIPTION = [
	...
	'Pass a `schema` to agent() whenever a later step branches on the result, so you get typed fields instead of prose. There is no resume: a failed run is simply re-run. Artifacts are saved under ~/.pi/agent/workflows/<runId>/ for inspection, kept private to the owner, and automatically removed after 14 days (active runs are never removed).',
	'Example:',
...
export const WORKFLOW_PROMPT_SNIPPET = 'Orchestrate isolated subagents from an inline JS script: phase()/agent()/parallel() with structured outputs and optional background execution';
...
export function buildWorkflowResultMessage(details: WorkflowDetails, runDir: string) {
	...
	if (details.agents.length > 0) {
		...
		for (const agent of details.agents) {
			...
			lines.push(`- [${agent.label}]${agent.phase ? ` (${agent.phase})` : ''} ${status}` + (agent.error ? ` — ${agent.error}` : ''));
		}
	}
...
export function buildBackgroundWorkflowFollowUp(options: { runId: string; status: WorkflowDetails['status']; result: string }) {
