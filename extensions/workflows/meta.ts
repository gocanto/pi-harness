...
function literalValue(node: Expression, depth = 0): unknown {
	if (depth > 8) {
		throw new Error('workflow metadata is nested too deeply');
	}

	if (isLiteral(node)) {
		if (node.value === null || typeof node.value === 'string' || typeof node.value === 'number' || typeof node.value === 'boolean') {
			return node.value;
		}
...
function sanitizeMeta(value: unknown): WorkflowMeta {
	...
	if (typeof raw.name === 'string') {
		meta.name = raw.name.slice(0, 160);
	}
