...
test('artifact transcript keeps the initial prompt, marker, and newest entries', () => {
	...
	const bounded = boundedArtifactTranscript(
		transcript,
		{
			maxBytes: 256,
			entryMaxBytes: 80,
...
test('live artifact persistence includes current agents and transcripts', () => {
	...
	try {
		...
		const transcripts = JSON.parse(readFileSync(join(directory, 'transcripts.json'), 'utf8')) as Record<string, TranscriptEntry[]>;
