 ...
 */
export function parseActivationEnv(value: string | undefined): boolean | undefined {
	if (value === undefined) {
		return undefined;
	}
 ...
 */
export function readActivationPreference(agentDir: string): boolean | undefined {
	try {
		const raw = JSON.parse(fs.readFileSync(activationPreferencePath(agentDir), 'utf8')) as PersistedActivationPreference;
 ...
 */
export function writeActivationPreference(agentDir: string, enabled: boolean): void {
	writeFileAtomic(
 ...
 */
export function resolveWorkflowActivation(options: { agentDir: string; env?: NodeJS.ProcessEnv }): ResolvedActivation {
	const envOverride = parseActivationEnv((options.env ?? process.env)[WORKFLOW_ACTIVATION_ENV_VAR]);
