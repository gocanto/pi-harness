...
import { safeStringify, toSerializable } from '@workflows/serialization.ts';
...
export function runWorkflowSandbox(options: RunWorkflowSandboxOptions) {
	...
	return new Promise<unknown>((resolve, reject) => {
		...
		const child = spawn(
			process.execPath,
			['--permission', `--allow-fs-read=${path.dirname(workerPath)}`, '--max-old-space-size=128', '--stack-size=2048', workerPath],
			{
				cwd: options.cwd,
				env: {
		...
		deadlineTimer = setTimeout(() => {
			finish(new Error(`Workflow exceeded its ${deadlineMs}ms execution deadline and was terminated`));
		}, deadlineMs);
		deadlineTimer.unref?.();
		...
		child.on('message', (raw: unknown) => {
			...
			if (raw.kind === 'phase') {
				if (typeof raw.payloadJson !== 'string' || byteLength(raw.payloadJson) > 4096) {
			...
			if (raw.kind === 'agent') {
				...
				const sendResult = (result: SandboxAgentResult) => {
					if (!activeAgentRequests.delete(id)) {
						return;
					}
					...
					const normalized = toSerializable(
						result,
						{
							maxDepth: 16,
							maxNodes: 10_000,
