...
function makeRun(
  ...
  if (summary !== undefined) {
    writeFileSync(join(runDir, "workflow.json"), JSON.stringify(summary));
  }
  return runDir;
}
...
test("cleanup removes runs older than the retention window", () => {
  ...
  try {
    const stale = makeRun(baseDir, "wf_stale", {
      status: "completed",
      finishedAt: NOW - RETENTION_MS - 1,
    });

    const removed = cleanupExpiredWorkflowRuns(baseDir, new Set(), {
...
test("cleanup preserves runs within the retention window", () => {
  const baseDir = mkdtempSync(join(tmpdir(), "pi-workflow-retention-"));
  try {
    const fresh = makeRun(baseDir, "wf_fresh", {
      status: "completed",
      finishedAt: NOW - RETENTION_MS + 1,
    });
...
test("cleanup never removes a run tracked as active, however old", () => {
  const baseDir = mkdtempSync(join(tmpdir(), "pi-workflow-retention-"));
  try {
    const active = makeRun(baseDir, "wf_active", {
      status: "running",
...
test("cleanup is idempotent across repeated calls", () => {
  const baseDir = mkdtempSync(join(tmpdir(), "pi-workflow-retention-"));
  try {
    makeRun(baseDir, "wf_stale", {
      status: "failed",
      finishedAt: NOW - RETENTION_MS - 1,
    });

    const first = cleanupExpiredWorkflowRuns(baseDir, new Set(), {
      retentionMs: RETENTION_MS,
      now: NOW,
    });
    const second = cleanupExpiredWorkflowRuns(baseDir, new Set(), {
      retentionMs: RETENTION_MS,
      now: NOW,
    });
...
test("cleanup tolerates a missing workflows base directory", () => {
  ...
  const removed = cleanupExpiredWorkflowRuns(missingDir, new Set(), {
    retentionMs: RETENTION_MS,
    now: NOW,
  });
  assert.deepEqual(removed, []);
