...
test('first-response watchdog aborts a silent provider request', async () => {
	...
	await assert.rejects(watchdog.waitFor(new Promise<never>(() => {})), /no assistant response event for fixture-model within 10 ms.*stalled/i);
...
test('workflow children guard structured, normal, and dynamically registered tools', async () => {
	...
	await assert.rejects(dynamic.execute('fixture', {}, undefined), /Tool call "dynamic_fixture" timed out after 10 ms\./);
...
function assistantTextMessage(text: string, overrides: Partial<AssistantSessionMessage> = {}): AssistantSessionMessage {
	return {
		role: 'assistant',
...
function createFakeSessionHandle(spec: FakeSessionSpec): FakeSessionHandle {
	...
	const createSession: CreateWorkflowAgentSession = async (creationOptions) => {
		...
		const tools = new Map<string, ToolDefinition>([...(creationOptions.customTools ?? []), ...(spec.extraTools ?? [])].map((tool) => [tool.name, tool] as const));
...
interface RunAgentEnv {
	...
	settingsManager: Awaited<ReturnType<typeof createChildResources>>['settingsManager'];
	modelRegistry: ModelRegistry;
}

/** Real, trust-gated resources rooted in a scratch directory; torn down after use. */
async function withRunAgentEnv<T>(run: (env: RunAgentEnv) => Promise<T>): Promise<T> {
	const cwd = await mkdtemp(
		path.join(tmpdir(), 'pi-workflow-runner-'),
	);
	...
	try {
		...
		return await run(
			{ cwd, loader, settingsManager, modelRegistry },
		);
	} finally {
		await rm(
			cwd,
...
test('runAgent completes a plain prompt, reports progress, and disposes the session once', async () => {
	await withRunAgentEnv(async (env) => {
		...
		assert.equal(
			outcome.transcript.some((entry) => entry.role === 'toolResult' && entry.name === 'first'),
			true,
		);

		assert.equal(progressUpdates.length, 3);
		assert.equal(progressUpdates[progressUpdates.length - 1]?.preview, 'Hello from the fixture agent.');
		...
		const handle = createFakeSessionHandle(
			{
				model: fixtureModel(),
				respond: async (control) => {
					const structuredTool = control.customTools?.find((tool) => tool.name === 'structured_output');

					assert.ok(structuredTool, 'expected runAgent to register a structured_output tool');

					const payload = { headline: 'release notes', ok: true };

					const result = await structuredTool.execute('call-1', payload, undefined, undefined, fixtureExtensionContext);
...
test('runAgent fails when a schema is supplied but structured_output is never called', async () => {
	await withRunAgentEnv(async (env) => {
		...
		assert.match(outcome.error ?? '', /finished without calling structured_output/);
		assert.equal(handle.disposeCount(), 1);
	});
});
...
test('runAgent enforces the per-tool call timeout on tools the session exposes', async () => {
	await withRunAgentEnv(async (env) => {
		...
		const handle = createFakeSessionHandle(
			{
				...
				respond: async (control) => {
					...
					await assert.rejects(guarded.execute('call-1', {}, undefined, undefined, fixtureExtensionContext), /Tool call "slow_tool" timed out after 5 ms\./);
		...
		assert.equal(outcome.ok, true);
		assert.equal(
			outcome.transcript.some((entry) => entry.role === 'toolResult' && entry.isError === true && /timed out after 5 ms/.test(entry.text)),
			true,
		);
		assert.equal(handle.disposeCount(), 1);
	});
...
test('runAgent reports session-creation failure without throwing or leaking a session', async () => {
	await withRunAgentEnv(async (env) => {
		...
		assert.equal(outcome.ok, false);
		assert.equal(outcome.aborted, false);
		assert.match(outcome.error ?? '', /Failed to create agent session: no auth configured for fixture-provider/);
...
test('runAgent disposes a session that fails to bind extensions during setup', async () => {
	await withRunAgentEnv(async (env) => {
		...
		assert.equal(outcome.ok, false);
		assert.match(outcome.error ?? '', /Failed to create agent session: extension bind exploded/);
		assert.equal(handle.disposeCount(), 1);
	});
});
...
test('progress snapshots stay correct across a long history, parallel tool calls, and mid-run compaction', async () => {
	await withRunAgentEnv(async (env) => {
		...
		const expectLatestSnapshotMatchesOracle = () => {
			...
			assert.deepEqual(latest.transcript, transcriptFromMessages(oracleMessages, oracleTimings));
		};
		...
		const handle = createFakeSessionHandle({
			...
			respond: async (control) => {
				...
				const turn2 = assistantTextMessage(
					'Turn two output.',
					{
						timestamp: 1_020,
					},
				...
				const turn3 = assistantTextMessage(
					'Turn three output.',
					{
						timestamp: 1_030,
					},
				...
				for (let i = 0; i < 210; i++) {
					const filler = assistantTextMessage(
						`Filler turn ${i}.`,
						{
							timestamp: 1_100 + i,
						},
				...
				const final = assistantTextMessage(
					'Final turn output.',
					{
						timestamp: 2_000,
					},
		...
		assert.deepEqual(outcome.usage, {
			...oracleUsage,
			...(outcome.usage.contextTokens === undefined ? {} : { contextTokens: outcome.usage.contextTokens }),
		});
		assert.deepEqual(outcome.transcript, transcriptFromMessages(oracleMessages, oracleTimings));
		assert.equal(handle.disposeCount(), 1);
	});
});
