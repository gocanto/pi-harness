...
function finishStdout(state: PreviewState, fullOutputPath: string) {
	if (!state.truncated) {
		state.preview += state.decoder.decode();
	}

	const totalBytes = state.totalBytes - state.trailingLineBreaks;

	const lineCount = totalBytes === 0 ? 0 : state.lineBreaks - state.trailingLineBreaks + 1;
...
function collectStderr<E, R>(stream: Stream.Stream<Uint8Array, E, R>) {
	return Stream.runFold(
		...
		() => Buffer.alloc(0),
		(captured, chunk) => {
			if (captured.byteLength >= STDERR_MAX_BYTES) {
				return captured;
			}
...
export function executeSearchProcess(options: { readonly command: string; readonly args: readonly string[]; readonly cwd: string; readonly tempPrefix: string }) {
	return Effect.gen(function* () {
			...
			const result = yield* Effect.all(
				{
					...
					stdout: process.stdout.pipe(
						Stream.tap((chunk) => Effect.sync(() => observeStdout(preview, chunk))),
		...
		}).pipe(Effect.ensuring(Effect.suspend(() => (retainDirectory ? Effect.void : fs.remove(directory, { recursive: true, force: true }).pipe(Effect.orDie)))));
	}).pipe(Effect.scoped);
}
