...
function truncationNotice(options: { content: string; outputLines: number; totalLines: number; outputBytes: number; totalBytes: number; fullOutputPath: string }) {
	return (
...
export async function formatOutput(output: string, options: FormatOutputOptions): Promise<FormattedOutput> {
	...
	const persist = options.persistFullOutput ?? ((full: string) => persistToTempFile(options.tempPrefix, full));
