...
export const FD_TOOL_DESCRIPTION =
	'Find files and directories by name with fd. Respects .gitignore by default. Results are limited to 1000 entries unless a higher limit is given; output is limited to 2000 lines or 50KB, and complete truncated output is saved to a temporary, private file that is removed when this session ends.';

export const FD_PROMPT_SNIPPET = 'Find files and directories by name with fd (fast, gitignore-aware).';
...
export const FD_PARAMETER_DESCRIPTIONS = {
	pattern: 'Regex matched against file names (or a glob when glob is true). Omit to list everything under path.',
...
export const RG_TOOL_DESCRIPTION =
	'Search file contents with ripgrep. Uses smart-case matching, respects .gitignore by default, and returns at most 100 matches per file unless a different limit is given. Output is limited to 2000 lines or 50KB; complete truncated output is saved to a temporary, private file that is removed when this session ends.';

export const RG_PROMPT_SNIPPET = 'Search file contents with ripgrep (fast regex content search).';
...
export const RG_PARAMETER_DESCRIPTIONS = {
	...
	file_type: "Only search files of this ripgrep type, e.g. 'ts', 'js', 'py', 'rust'.",
	case_sensitive: 'true forces case-sensitive matching, false forces case-insensitive. Defaults to smart-case.',
