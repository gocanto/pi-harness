...
export function resolveBinary(spec: ToolSpec, binDir: string, target: PlatformTarget, env: BinaryEnv): Effect.Effect<ResolvedBinary, UnsupportedPlatformError | InstallError> {
...
export function readBoundedResponse<E, R>(
	...
	return Effect.gen(function* () {
		...
		if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
			return yield* Effect.fail(new Error(`download exceeds the ${maxBytes}-byte size limit`));
		}
		...
		const result = yield* Stream.runFoldEffect(
			...
			(accumulator, chunk) => {
				...
				if (totalBytes > maxBytes) {
					return Effect.fail(new Error(`download exceeds the ${maxBytes}-byte size limit`));
				}
	...
	return Effect.gen(function* () {
		...
		for (let redirects = 0; redirects <= MAX_DOWNLOAD_REDIRECTS; redirects++) {
			if (url.protocol !== 'https:') {
				return yield* Effect.fail(new Error(`refusing non-HTTPS download URL: ${url.href}`));
			}
			...
			const result = yield* Effect.scoped(
				Effect.gen(function* () {
					...
					if ([301, 302, 303, 307, 308].includes(response.status)) {
						...
						if (!location) {
							return yield* Effect.fail(new Error(`redirect from ${url.href} had no location header`));
						}

						if (redirects === MAX_DOWNLOAD_REDIRECTS) {
							return yield* Effect.fail(new Error(`download exceeded ${MAX_DOWNLOAD_REDIRECTS} redirects`));
						}

						if (!URL.canParse(location, url)) {
							return yield* Effect.fail(new Error(`download returned an invalid redirect URL: ${location}`));
						}
					...
					if (response.status < 200 || response.status >= 300) {
						return yield* Effect.fail(new Error(`download failed with HTTP ${response.status}`));
					}
	...
	install: (asset, destination) => {
		const install = Effect.gen(function* () {
			if (!URL.canParse(asset.url)) {
				return yield* Effect.fail(new Error(`invalid download URL: ${asset.url}`));
			}
			...
			if (digest !== asset.sha256) {
				return yield* Effect.fail(new Error(`SHA-256 mismatch for ${asset.fileName}: expected ${asset.sha256}, received ${digest}`));
			}
			...
			const tarExitCode = yield* Effect.scoped(
				Effect.gen(function* () {
					const tar = yield* ChildProcess.make('tar', ['-xzf', archivePath, '-C', workDir], { stdin: 'ignore', stdout: 'ignore', stderr: 'ignore' });
			...
			if (tarExitCode !== ChildProcessSpawner.ExitCode(0)) {
				return yield* Effect.fail(new Error(`tar failed with exit code ${tarExitCode}`));
			}
			...
			yield* Effect.addFinalizer(() => fs.remove(stagedDestination, { force: true }).pipe(Effect.orDie));
