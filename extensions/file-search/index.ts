...
import { formatCapturedOutput, type CapturedOutput } from '@file-search/src/output.ts';
import { discardCapturedOutput, executeSearchProcess } from '@file-search/src/process.ts';
import { createSessionCaptureRegistry } from '@file-search/src/session-captures.ts';
import type { AgentToolResult, ExtensionAPI, ExtensionContext } from '@earendil-works/pi-coding-agent';
import { buildFdArgs, buildRgArgs, FD_MAX_DEPTH_LIMIT, FD_MAX_LIMIT, RG_MAX_CONTEXT, RG_MAX_COUNT_LIMIT } from '@file-search/src/args.ts';
import { currentTarget, liveBinaryEnv, repositoryBinDir, resolveBinary, TOOL_SPECS, type BinaryEnv, type BinarySource, type PlatformTarget, type ResolvedBinary } from '@file-search/src/binaries.ts';
...
import {
	...
	RG_PROMPT_SNIPPET,
	RG_TOOL_DESCRIPTION,
} from '@file-search/src/prompt.ts';

export function makeBinaryInitializers(binDir: string, target: PlatformTarget, env: BinaryEnv) {
	return {
		fd: Effect.runSync(Effect.cached(resolveBinary(TOOL_SPECS.fd, binDir, target, env))),
		rg: Effect.runSync(Effect.cached(resolveBinary(TOOL_SPECS.rg, binDir, target, env))),
	};
}
...
export function installNotifications(binaries: readonly ResolvedBinary[]) {
	return binaries
		...
		.map((binary) => `file-search: no system ${binary.tool} found — downloaded ${binary.tool} ${binary.version ?? ''}`.trimEnd() + ` to ${repositoryBinDir()}`);
}
...
export default function fileSearchTools(pi: ExtensionAPI) {
	...
	pi.on('session_start', async (_event, ctx) => {
			...
			Effect.gen(function* () {
				...
				for (const tool of ['fd', 'rg'] as const) {
					...
					if (Exit.isSuccess(toolExit)) {
						for (const message of installNotifications(
							[toolExit.value],
						)) {
							ctx.ui.notify(message, 'info');
						}
					} else {
						ctx.ui.notify(`file-search ${tool} setup failed: ${causeMessage(toolExit.cause)}`, 'error');
					}
				}
			}),
		);
		...
		if (Exit.isFailure(exit) && ctx.hasUI && !notified) {
			notified = true;
			ctx.ui.notify(`file-search setup failed: ${causeMessage(exit.cause)}`, 'error');
		}
	});
		...
		return Effect.gen(function* () {
			...
			Effect.mapError((error) => {
				...
				return new SearchError({
					message: error._tag === 'TimeoutError' ? `${tool} timed out.` : error instanceof Error ? error.message : String(error),
				});
			}),
...
function fdParameters() {
	return Type.Object({
		pattern: Type.Optional(Type.String({ description: FD_PARAMETER_DESCRIPTIONS.pattern })),
		path: Type.Optional(Type.String({ description: FD_PARAMETER_DESCRIPTIONS.path })),
		type: Type.Optional(
			StringEnum(['file', 'directory', 'symlink'] as const, {
				description: FD_PARAMETER_DESCRIPTIONS.type,
			}),
		),
		extension: Type.Optional(Type.String({ description: FD_PARAMETER_DESCRIPTIONS.extension })),
		glob: Type.Optional(Type.Boolean({ description: FD_PARAMETER_DESCRIPTIONS.glob })),
		hidden: Type.Optional(Type.Boolean({ description: FD_PARAMETER_DESCRIPTIONS.hidden })),
...
function rgParameters() {
	return Type.Object({
		...
		path: Type.Optional(Type.String({ description: RG_PARAMETER_DESCRIPTIONS.path })),
		glob: Type.Optional(Type.String({ description: RG_PARAMETER_DESCRIPTIONS.glob })),
		file_type: Type.Optional(Type.String({ description: RG_PARAMETER_DESCRIPTIONS.file_type })),
		case_sensitive: Type.Optional(Type.Boolean({ description: RG_PARAMETER_DESCRIPTIONS.case_sensitive })),
		fixed_strings: Type.Optional(Type.Boolean({ description: RG_PARAMETER_DESCRIPTIONS.fixed_strings })),
		hidden: Type.Optional(Type.Boolean({ description: RG_PARAMETER_DESCRIPTIONS.hidden })),
