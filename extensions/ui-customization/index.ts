...
import { emptyGitInfoState, emptyModelInfoState, GIT_INFO_CHANNEL, MODEL_INFO_CHANNEL, REFRESH_CHANNEL, isGitInfoState, isModelInfoState } from '@shared/dashboard-state.ts';
...
function sampleGradient(position: number) {
	...
	return [mix(start[0], end[0], amount), mix(start[1], end[1], amount), mix(start[2], end[2], amount)] satisfies Rgb;
}
...
function gradientText(text: string, phase: number) {
	...
	return characters.map((character, index) => (character === ' ' ? character : foreground(
		sampleGradient(index / span + phase),
		character,
	))).join('');
}

function hasChildren(component: RenderableNode): component is RenderableNode & { children: RenderableNode[] } {
...
function hideThemesSection(component: RenderableNode) {
	...
	for (let index = 0; index < component.children.length; index += 1) {
		...
		if (firstLine === '[Themes]') {
			const removeCount = component.children[index + 1] && renderedText(component.children[index + 1]!).trim() === '' ? 2 : 1;
...
export default function uiCustomization(pi: ExtensionAPI) {
	...
	function install(ctx: ExtensionContext) {
		...
		ctx.ui.setHeader((tui) => {
			...
			return {
				render(width: number) {
					const art = TITLE_LINES.map((line, row) => center(
						gradientText(line, row * 0.045),
						width,
					));
		...
		ctx.ui.setFooter((tui, theme, footerData: ReadonlyFooterDataProvider) => {
			...
			return {
				...
				render(width: number) {
					...
					let git = gitInfo.branch ? `${gitInfo.branch} · ${gitInfo.changedFiles} ${fileLabel} changed` : '';

					if (gitInfo.pullRequest) {
						const prLabel = `PR #${gitInfo.pullRequest.number}`;

						const linkedPr = getCapabilities().hyperlinks ? hyperlink(prLabel, gitInfo.pullRequest.url) : prLabel;

						git += ` · ${linkedPr}`;
					}

					const contextPercent = modelInfo.contextPercent === null ? '?' : `${Math.round(modelInfo.contextPercent)}`;
					const contextWindow = modelInfo.contextWindow > 0 ? formatTokens(modelInfo.contextWindow) : '?';
					const tps = modelInfo.tokensPerSecond === null ? '— tok/s' : `${Math.round(modelInfo.tokensPerSecond)} tok/s`;
					const usage = `${contextPercent}%/${contextWindow} · $${modelInfo.cost.toFixed(2)} · ${tps}`;
					const model = modelInfo.provider ? `${modelInfo.provider}/${modelInfo.modelId} · ${modelInfo.thinking}` : modelInfo.modelId;
