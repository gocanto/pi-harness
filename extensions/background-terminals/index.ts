...
export default function (pi: ExtensionAPI) {
	...
	const resultDelivery = new DeferredResultDelivery<TerminalSnapshot>();
	...
	pi.registerTool({
		...
		async execute() {
			...
			const text = terminals.length === 0 ? 'No background terminals.' : terminals.map((snap) => describeTerminal(snap)).join('\n');
	...
	pi.registerMessageRenderer('background-terminal-result', (message, { expanded }, theme) => {
		...
		const icon = failed ? theme.fg('error', 'x') : killed ? theme.fg('muted', '■') : theme.fg('success', '■');
		const how = killed ? 'killed' : (details.signal ?? `exit ${details.exitCode ?? '?'}`);
		const header = `${icon} ` + theme.fg('accent', theme.bold(`terminal ${details.id ?? '?'}`)) + theme.fg('muted', ` · ${details.title ?? ''} · ${how}`);
		...
		handler: async (_args, ctx) => {
			...
			if (ctx.mode !== 'tui') {
				if (ctx.hasUI) {
					...
					ctx.ui.notify(terminals.length === 0 ? 'No background terminals.' : terminals.map((snap) => describeTerminal(snap)).join('\n'), 'info');
				}
