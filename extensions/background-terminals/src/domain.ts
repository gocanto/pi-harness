...
/** Pure presentation rules for terminal snapshots. */
export class TerminalSnapshotFormatter {
	/** Format elapsed lifetime using the current time for running terminals. */
	static elapsed(snap: TerminalSnapshot) {
	...
	/** Format the terminal's final exit reason. */
	static exit(snap: TerminalSnapshot) {
...
export function formatElapsed(snap: TerminalSnapshot) {
	return TerminalSnapshotFormatter.elapsed(snap);
}

/** Backward-compatible presentation helper. */
export function formatExit(snap: TerminalSnapshot) {
	return TerminalSnapshotFormatter.exit(snap);
}
