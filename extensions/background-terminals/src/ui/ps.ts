...
import { formatElapsed, formatExit, type TerminalSnapshot } from '@background-terminals/src/domain.ts';
import type { TerminalReadModel } from '@background-terminals/src/manager.ts';
import { createOutputLineCache, sanitizeText } from '@background-terminals/src/ui/output-view.ts';
...
export async function openTerminalPicker(ctx: ExtensionCommandContext, view: TerminalReadModel) {
	...
	while (true) {
		...
		const picked = await ctx.ui.custom<string | null>((tui, theme, keybindings, done) => new TerminalDashboard(tui, theme, keybindings, view, selection, done), {
			overlay: true,
		...
		await ctx.ui.custom<null>((tui, theme, keybindings, done) => new TerminalDetailView(tui, theme, keybindings, picked, view, done), {
			overlay: true,
			overlayOptions: { anchor: 'center', width: '100%', maxHeight: '100%' },
		});
...
export function reconcileDashboardSelection(selection: DashboardSelection, terminals: ReadonlyArray<Pick<TerminalSnapshot, 'id'>>) {
	const stableIndex = selection.id ? terminals.findIndex((snap) => snap.id === selection.id) : -1;

	selection.index = stableIndex >= 0 ? stableIndex : Math.min(Math.max(0, selection.index), Math.max(0, terminals.length - 1));
...
class TerminalDashboard implements Component {
	...
	constructor(tui: TUI, theme: Theme, keybindings: KeybindingsManager, view: TerminalReadModel, selection: DashboardSelection, done: (value: string | null) => void) {
		this.tui = tui;
		this.theme = theme;
	...
	private borderSegment(width: number, title: string): string {
		...
		const label = title ? ` ${truncateToWidth(
			title,
			Math.max(0, width - 3),
		)} ` : '';
		...
		return theme.fg('border', '─') + (label ? theme.fg('text', label) : '') + theme.fg('border', '─'.repeat(Math.max(0, width - 1 - labelWidth)));
	}
	...
	render(width: number): string[] {
		...
		const headerRight = theme.fg('muted', `${terminals.length} terminal${terminals.length === 1 ? '' : 's'}`);
		...
		lines.push(theme.fg('border', '╭') + this.borderSegment(innerWidth, `terminals · ${running} running / ${terminals.length}`) + theme.fg('border', '╮'));
		...
		// Bottom border
		lines.push(theme.fg('border', '╰') + theme.fg('border', '─'.repeat(Math.max(0, innerWidth))) + theme.fg('border', '╯'));
	...
	private renderRows(terminals: ReadonlyArray<TerminalSnapshot>, width: number, height: number): string[] {
		const theme = this.theme;
		const out: string[] = [];
		...
		if (terminals.length > height) {
			start = Math.min(Math.max(0, this.selection.index - Math.floor(height / 2)), terminals.length - height);
		}
		...
		for (let i = 0; i < visible.length; i++) {
			...
			const title = isSelected ? theme.fg('accent', oneLine(snap.title)) : theme.fg('text', oneLine(snap.title));
...
class TerminalDetailView implements Component {
	...
	constructor(tui: TUI, theme: Theme, keybindings: KeybindingsManager, id: string, view: TerminalReadModel, done: (value: null) => void) {
		this.tui = tui;
		this.theme = theme;
		this.keybindings = keybindings;
		this.id = id;
	...
	render(width: number): string[] {
		...
		lines.push(truncateToWidth(theme.fg('dim', '$ ') + theme.fg('text', oneLine(snap.command)), width));
		lines.push(border);
		...
		const tab = (name: 'stdout' | 'stderr', size: number) => (name === active ? theme.fg('accent', theme.bold(`${name} (${formatSize(size)})`)) : theme.fg('dim', `${name} (${formatSize(size)})`));

		lines.push(truncateToWidth(`  ${tab('stdout', snap.stdout.totalBytes)}${theme.fg('dim', ' | ')}${tab('stderr', snap.stderr.totalBytes)}${theme.fg('dim', '  — t to switch')}`, width));
		...
		if (buffer.truncatedBytes > 0) {
			noteRows.push(truncateToWidth(theme.fg('dim', `first ${formatSize(buffer.truncatedBytes)} dropped from view — full log: ${buffer.spillPath ?? '(unavailable)'}`), width));
		}
