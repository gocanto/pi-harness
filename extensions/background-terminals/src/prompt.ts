...
import { MAX_RUNNING, type KillResult } from '@background-terminals/src/manager.ts';
...
export const BG_START_PROMPT_SNIPPET = "Run a long-lived shell command in the background (dev servers, builds, watchers); output is captured and you're notified on exit";
...
export const BG_START_PARAMETER_DESCRIPTIONS = {
	command: 'Shell command line to run in the background (sh -c on POSIX, cmd.exe /d /s /c on Windows). It receives no stdin (EOF immediately); interactive commands will not work.',
...
export const BG_STATUS_TOOL_DESCRIPTION = "Peek at a background terminal's status and current output (tail-truncated) without blocking. If the terminal already exited, this returns its final state.";

export const BG_STATUS_PARAMETER_DESCRIPTIONS = {
	id: 'Terminal id, e.g. "bt-1"',
};

export const BG_LIST_TOOL_DESCRIPTION = 'List all background terminals (running and settled) with pid, elapsed time, exit status, and output sizes.';
