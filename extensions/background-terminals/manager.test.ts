...
async function withManager(run: (manager: TerminalManagerShape, runtime: ReturnType<typeof createTerminalRuntime>) => Promise<void>) {
...
test('happy path: stdout and stderr captured separately, settles done, hook fires once unconsumed', async () => {
	await withManager(async (manager, runtime) => {
		...
		manager.view.setOnSettled((snap, consumed) => settled.push({ id: snap.id, status: snap.status, consumed }));

		const snap = await runTool(
			runtime,
			manager.start({
				command: nodeCmd('process.stdout.write("out-line\\n"); process.stderr.write("err-line\\n");'),
				title: 'happy',
				cwd,
			}),
		);
...
test(
	...
	async () => {
		await withManager(async (manager, runtime) => {
			...
			assert.ok(await pollUntil(() => (manager.view.get(snap.id)?.stdout.text ?? '').includes('ready')), 'child installed its SIGTERM handler');
...
test('concurrent overlapping multi-id kills observe each settlement exactly once', async () => {
	await withManager(async (manager, runtime) => {
		...
		const reports = await runTool(
			runtime,
			Effect.all([manager.kill([first.id, second.id, first.id]), manager.kill([second.id, first.id])], { concurrency: 'unbounded' }),
		);
...
test('kill terminates the whole process tree (grandchildren die)', { skip: process.platform === 'win32' }, async () => {
	await withManager(async (manager, runtime) => {
		...
		assert.ok(await pollUntil(() => (manager.view.get(snap.id)?.stdout.text ?? '').includes('child:')), 'grandchild pid was printed');
		...
		assert.ok(await pollUntil(() => fs.readFileSync(sentinel, 'utf8') !== heartbeatBefore), 'heartbeat belongs to the live grandchild');
		...
		assert.ok(await pollUntil(() => processGone(grandchild)), 'grandchild process is gone after group kill');

		const stoppedAt = fs.readFileSync(sentinel, 'utf8');

		await new Promise((resolve) => setTimeout(resolve, 100));

		assert.equal(fs.readFileSync(sentinel, 'utf8'), stoppedAt, 'the unique grandchild heartbeat stopped');
...
test(
	...
	async () => {
		await withManager(async (manager, runtime) => {
			...
			assert.ok(await pollUntil(() => (manager.view.get(snap.id)?.stdout.text ?? '').includes('child:')), 'descendant pid was printed');
			...
			assert.ok(await pollUntil(() => manager.view.get(snap.id)?.status !== 'running', 7_000), 'entry settled after the bounded post-exit grace');

			assert.equal(manager.view.get(snap.id)?.status, 'done');
			assert.equal(manager.view.get(snap.id)?.exitCode, 0);

			assert.ok(await pollUntil(() => processGone(grandchild)), 'surviving process-group descendant was reaped');
		});
	},
);
...
test(
	...
	async () => {
		await withManager(async (manager, runtime) => {
			...
			assert.ok(await pollUntil(() => (manager.view.get(snap.id)?.stdout.text ?? '').includes('child:')));
...
test('concurrency cap rejects an extra start; a failed spawn releases its slot', async () => {
	await withManager(async (manager, runtime) => {
		...
		await assert.rejects(runTool(runtime, manager.start({ command: 'true', title: 'extra', cwd })), new RegExp(`Max ${MAX_RUNNING} background terminals`));

		// Free one slot; a bogus binary settles as failed near-instantly (the
		// 'error'/'exit' path), leaving the slot free again.
		await runTool(
			runtime,
			manager.kill([spawns[0].id]),
		);
...
test('runtime.dispose kills running processes; no settle hook fires after dispose', async () => {
	...
	await assert.rejects(runTool(runtime, manager.start({ command: 'true', title: 'late', cwd })), /shutting down|disposed/);
});
...
test('pruning drops the oldest settled entries past MAX_TRACKED, never running ones', async () => {
	await withManager(async (manager, runtime) => {
		...
		const [historical] = await runTool(
			runtime,
			manager.kill([settledIds[0]]),
		);
...
test('the spill file holds the complete capture when the settle hook fires, beyond the in-memory cap', async () => {
	await withManager(async (manager, runtime) => {
		...
		const snap = await runTool(
			runtime,
			manager.start({
				command: nodeCmd(`const s = "x".repeat(${chunk}); for (let i = 0; i < ${writes}; i++) process.stdout.write(s);`),
				title: 'firehose',
				cwd,
			}),
		);
		...
		assert.ok(Buffer.byteLength(done.stdout.text) <= 2 * 1024 * 1024, 'retained text within the cap');
		if (done.stdout.spillPath) {
			assert.equal(spillSizeAtSettle, totalBytes, 'spill file was fully flushed before the settle hook');
		}
	});
});
			...
			manager.start({
				command:
					process.platform === 'win32'
						...
						: `exec ${nodeCmd('process.on("SIGTERM", () => process.stdout.write("term\\n")); process.stdout.write("ready\\n"); setInterval(() => {}, 1000);')}`,
				title: 'abort-race',
				cwd,
			}),
		);
...
test('status returns the snapshot and rejects unknown ids with the known list', async () => {
	await withManager(async (manager, runtime) => {
		...
		const seen = await runTool(
			runtime,
			manager.status(snap.id),
		);

		assert.equal(seen.id, snap.id);

		await assert.rejects(runTool(runtime, manager.status('bt-999')), /Unknown terminal id "bt-999"\. Known: bt-1\./);
	});
});
