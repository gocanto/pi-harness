      ...
      run: (_command, args) =>
        Effect.sync(() => {
          ...
          if (args[0] === "rev-parse" && args.includes("--show-toplevel")) {
            return (
              toplevelResult ?? { code: 0, stdout: `${root}\n`, stderr: "" }
            );
          }
...
function runLoadChangedFiles(
  cwd: string,
  fixture: ReturnType<typeof makeFixture>,
) {
  return Effect.runPromise(
    loadChangedFiles(cwd).pipe(Effect.provide(fixture.layer)),
  );
}

function runLoadFileDiff(
  repoRoot: string,
  file: { rawPath: string; status: string },
  hasHead: boolean,
  fixture: ReturnType<typeof makeFixture>,
) {
  return Effect.runPromise(
    loadFileDiff(repoRoot, file, hasHead).pipe(Effect.provide(fixture.layer)),
  );
}
...
test("loadChangedFiles: the initial pass is cheap stats only, with no per-file diff commands", async () => {
  ...
  assert.deepEqual(
    result.files.map(
      ({ name, path, rawPath, status, additions, deletions }) => ({
        name,
        path,
        rawPath,
        status,
  ...
  const numstatCalls = fixture.calls.filter(
    (args) => args[0] === "diff" && args.includes("--numstat"),
  );
  assert.equal(numstatCalls.length, 5);

  // The initial listing must never load a full textual diff.
  const fullDiffCalls = fixture.calls.filter((args) =>
    args.includes("--unified=3"),
  );
  assert.equal(fullDiffCalls.length, 0);

  const untrackedStat = numstatCalls.find(
    (args) => args[args.length - 1] === "untracked.ts",
  )!;
  assert.ok(untrackedStat.includes("--no-index"));
  assert.ok(untrackedStat.includes("/dev/null"));

  const modifiedStat = numstatCalls.find(
    (args) => args[args.length - 1] === "modified.ts",
  )!;
  assert.ok(!modifiedStat.includes("--no-index"));
  assert.ok(modifiedStat.includes("HEAD"));

  const weirdStat = numstatCalls.find(
    (args) => args[args.length - 1] === "src/weird\ttab.ts",
  )!;
...
test("loadChangedFiles: no HEAD forces no-index diffs even for tracked paths", async () => {
  ...
  const statCall = fixture.calls.find(
    (args) => args[0] === "diff" && args.includes("--numstat"),
  )!;
...
test("loadFileDiff: selecting a file loads only that file's diff", async () => {
  ...
  assert.ok(result !== null);
  assert.equal(
    fixture.calls.filter((args) => args.includes("--unified=3")).length,
    0,
    "listing changed files must not load any diff text",
  );
  ...
  const outcome = await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.forkChild(
        loadFileDiff("/repo", { rawPath: "big.ts", status: "M " }, true).pipe(
