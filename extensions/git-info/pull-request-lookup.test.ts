...
import { lookupPullRequest, PullRequestQueryTracker, type PullRequestLookupResult } from './src/pull-request-lookup.ts';
...
function fixture(result: CommandResult) {
	return Layer.succeed(CommandRunner, CommandRunner.of({ run: () => Effect.succeed(result) }));
}

function runWithFixture<A>(effect: Effect.Effect<A, never, CommandRunner>, result: CommandResult) {
 ...
 */
function pollOnce(tracker: PullRequestQueryTracker, branch: string, force: boolean, layer: Layer.Layer<CommandRunner>, options: { stale?: boolean } = {}) {
