...
export const SUBAGENT_SPAWN_PROMPT_SNIPPET = 'Spawn a background subagent on a chosen harness (pi, Claude Code, or Codex; own context, normal tools) for a self-contained task';
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
	...
	harness: 'Harness to run the subagent on: "pi" (in-process pi session; inherits this environment), "claude" (Claude Code), or "codex" (Codex CLI). Choose deliberately per task.',
	workingDir: 'Trusted working directory for the autonomous child (default: current working directory)',
	model: 'Model hint, interpreted by the chosen harness (pi: "provider/model-id" or model id; claude: model alias like "sonnet"/"opus"; codex: model slug). Omit for the harness default (pi inherits the current model).',
	reasoningEffort:
...
export function buildSubagentSpawnResult(options: { id: string; title: string; harness: string; modelLabel: string; cwd: string }) {
	return (
...
export const SUBAGENT_CANCEL_TOOL_DESCRIPTION = 'Cancel one or more running subagents. This aborts their active work but preserves their partial session transcripts on disk.';
...
export const SUBAGENT_CHECK_TOOL_DESCRIPTION = "Peek at a subagent's status and recent activity without blocking. Does not consume its result.";
...
export const SUBAGENT_LIST_TOOL_DESCRIPTION = 'List all subagents (running and finished) with their harness and status.';

/** Builds the child completion/failure wrapper injected into the parent model's context. */
export function buildSubagentResultMessage(options: { id: string; title: string; status: 'running' | 'done' | 'error'; errorText?: string; output: string }) {
