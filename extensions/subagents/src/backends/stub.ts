...
import { SendError } from '@subagents/src/domain.ts';

import type { BackendName, QueuedMessage, SpawnTask, SubagentEvent, SubagentMeta } from '@subagents/src/domain.ts';
...
const makeStubSession = (profile: StubProfile, task: SpawnTask): Effect.Effect<SubagentSession, never, Scope.Scope> =>
	Effect.gen(function* () {
		...
		const queuedView = (): ReadonlyArray<QueuedMessage> => state.pending.map((text) => ({ text, kind: 'steer' as const }));
