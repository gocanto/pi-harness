...
import { formatElapsed, type SubagentSnapshot } from '@subagents/src/domain.ts';
import { formatContextUtilization } from '@subagents/src/format.ts';
...
export async function openSubagentTakeover(ctx: ExtensionCommandContext, view: SubagentReadModel, id: string, options?: TakeoverOptions) {
	if (!view.get(id)) {
		return;
	}

	await ctx.ui.custom<null>((tui, theme, keybindings, done) => new TakeoverView(tui, theme, keybindings, id, view, done, options), {
		overlay: true,
...
export async function openSubagentPicker(ctx: ExtensionCommandContext, view: SubagentReadModel) {
	...
	while (true) {
		...
		const picked = await ctx.ui.custom<string | null>((tui, theme, keybindings, done) => new SubagentDashboard(tui, theme, keybindings, view, selection, done), {
			overlay: true,
			overlayOptions: { anchor: 'center', width: '100%', maxHeight: '100%' },
		});
...
class SubagentDashboard implements Component {
	...
	constructor(tui: TUI, theme: Theme, keybindings: KeybindingsManager, view: SubagentReadModel, selection: DashboardSelection, done: (value: string | null) => void) {
		this.tui = tui;
		this.theme = theme;
	...
	render(width: number): string[] {
		...
		lines.push(theme.fg('border', '╭') + this.borderSegment(innerWidth, `agents · ${settled}/${subs.length}`) + theme.fg('border', '╮'));
...
class TakeoverView implements Component, Focusable {
	...
	constructor(tui: TUI, theme: Theme, keybindings: KeybindingsManager, id: string, view: SubagentReadModel, done: (value: null) => void, options?: TakeoverOptions) {
		this.tui = tui;
		this.theme = theme;
		this.keybindings = keybindings;
		this.id = id;
