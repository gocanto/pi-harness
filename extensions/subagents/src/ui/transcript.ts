...
import type { SubagentSnapshot, TranscriptItem } from '@subagents/src/domain.ts';
...
function renderAssistantItem(theme: Theme, item: Extract<TranscriptItem, { kind: 'assistant' }>, width: number, out: string[]) {
	for (const part of item.parts) {
		if (part.type === 'text') {
			...
			const line = theme.fg('muted', '→ ') + theme.fg('toolTitle', part.name) + (preview && preview !== '{}' ? theme.fg('dim', ` ${preview}`) : '');
...
function renderToolResultItem(theme: Theme, item: Extract<TranscriptItem, { kind: 'toolResult' }>, width: number, out: string[]) {
	const firstLine =
