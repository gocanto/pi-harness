...
const p: Promise<number> = Effect.runPromise(program);
const pe: Promise<Exit.Exit<number, never>> = Effect.runPromiseExit(program);
const s: number = Effect.runSync(program);
...
const runnable = use.pipe(Effect.provide(AppLayer));
...
const handled: Effect.Effect<string, TimeoutError> = risky.pipe(Effect.catchTag('SpawnError', (e) => Effect.succeed(`spawn failed: ${e.command}`)));
...
const b = Effect.tryPromise((signal) => fetch(
	'https://x',
	{ signal },
));
...
```ts
import { Cause, Deferred, Effect, Fiber, FiberMap, PubSub, Queue, Ref, Stream, Exit } from 'effect';
