...
test(
	...
	async (t) => {
		...
		try {
			...
			const started = await runTool(runtime, manager.spawn('claude', task('Write a detailed 10,000-word essay about the history of computing.')));

			const streamDeadline = Date.now() + 15_000;

			while (manager.view.get(started.id)?.status === 'running' && !manager.view.get(started.id)?.liveAssistant?.text && Date.now() < streamDeadline) {
