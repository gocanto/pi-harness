...
test('spawn origin propagates to ids, snapshots, and settlement', async () => {
	await withManager(async (manager, runtime) => {
		...
		await runTool(
			runtime,
			manager.cancel([model.id, btw.id]),
		);
...
test('the global concurrency cap includes by-the-way sessions', async () => {
	await withManager(async (manager, runtime) => {
		const tasks: SpawnTask[] = [{ ...task('side question'), origin: 'btw' }, task('Task 2'), task('Task 3'), task('Task 4')];
...
test('the concurrency cap rejects a fifth running subagent', async () => {
	await withManager(async (manager, runtime) => {
		const spawns = await runTool(
			runtime,
			Effect.forEach([1, 2, 3, 4], (n) => manager.spawn('codex', task(`Task ${n}`)), { concurrency: 'unbounded' }),
		);

		assert.equal(spawns.length, 4);

		await assert.rejects(runTool(runtime, manager.spawn('codex', task('Task 5'))), /Max 4 subagents/);
	});
});
