...
function describeSubagent(snap: SubagentSnapshot) {
	const details = [`${snap.backend}: ${snap.meta.modelLabel ?? '?'}`, formatContextUtilization(snap.usage), formatElapsed(snap), snap.cwd].filter(Boolean);
...
export default function (pi: ExtensionAPI) {
	...
	const resultDelivery = new DeferredResultDelivery<SubagentSnapshot>();
	...
	const deliverBtwResult = (snap: SubagentSnapshot) => {
		...
		ui?.notify(
			snap.status === 'error' ? `by the way “${snap.title}” failed — reopen it with /subagents` : `by the way “${snap.title}” answered — reopen it with /subagents`,
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (!projectTrusted) {
				throw new Error(`working_dir is not trusted: ${cwd}. Trust this directory in pi (or spawn inside the trusted project directory) before retrying.`);
			}
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal) {
			...
			const lines = report.map((entry) => (entry.cancelled ? `Cancelled ${entry.id} "${entry.title}".` : `${entry.id} "${entry.title}" was already ${entry.status}.`));
	...
	pi.registerMessageRenderer('subagent-result', (message, { expanded }, theme) => {
		...
		const header = `${icon} ` + theme.fg('accent', theme.bold(`subagent ${details.id ?? '?'}`)) + theme.fg('muted', ` · ${details.title ?? ''} · ${failed ? 'failed' : 'finished'}`);
	...
	pi.registerEntryRenderer<BtwResultData>('btw-result', (entry, { expanded }, theme) => {
		...
		const header = `${icon} ` + theme.fg('accent', theme.bold(`by the way · ${data?.title ?? '?'}`)) + theme.fg('muted', ` · ${failed ? 'failed' : 'answered'} · ${data?.id ?? '?'}`);

		const body = [data?.errorText ? `Error: ${data.errorText}` : '', data?.answer ?? '(no answer)'].filter(Boolean).join('\n\n');
	...
	pi.registerCommand('subagents', {
		...
		handler: async (_args, ctx) => {
			if (ctx.mode !== 'tui') {
				if (ctx.hasUI) {
					ctx.ui.notify('Subagent takeover is only available in the TUI', 'error');
				}
