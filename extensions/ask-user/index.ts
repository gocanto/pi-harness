...
import { ASK_USER_PARAMETER_DESCRIPTIONS, ASK_USER_PROMPT_GUIDELINES, ASK_USER_PROMPT_SNIPPET, ASK_USER_TOOL_DESCRIPTION, buildAskUserResultMessage } from '@ask-user/prompt.ts';
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (params.options.length < MIN_OPTIONS || params.options.length > MAX_OPTIONS) {
				throw new Error(`ask_user requires between ${MIN_OPTIONS} and ${MAX_OPTIONS} options (got ${params.options.length}). Retry with a valid number of options.`);
			}
				...
				ctx.ui.custom<SelectionResult>((tui, theme, _kb, done) => {
					...
					function render(width: number): string[] {
						...
						if (editMode) {
							...
							add(
								theme.fg('dim', ` ↑↓ or 1-${allOptions.length} select • Enter confirm • Esc dismiss`),
							);
						}
