...
test('parses strict recap JSON', () => {
	assert.deepEqual(parseRecapResponse('{"recap":"Updated config and ran focused tests.","next":"Review the diff."}'), {
...
test('rejects malformed or incomplete output', () => {
	...
	assert.throws(() => parseRecapResponse('{"recap":"done","next":"nothing","extra":"not allowed"}'), /valid recap JSON/);
});

test('strips terminal control sequences from recap fields', () => {
	assert.deepEqual(parseRecapResponse('{"recap":"Updated \\u001b[31mconfig\\u001b[0m.","next":"Review it.\\u0007"}'), { recap: 'Updated config.', next: 'Review it.' });
});
