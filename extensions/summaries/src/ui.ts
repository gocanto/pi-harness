...
import type { ReasoningLevel, SummaryConfig } from '@summaries/src/config.ts';
import type { RunRecap } from '@summaries/src/summarizer.ts';

import { getMarkdownTheme, ThinkingSelectorComponent, type ExtensionCommandContext, type Theme } from '@earendil-works/pi-coding-agent';
...
class RecapCard {
	...
	render(width: number) {
		...
		const title = this.theme.fg('accent', '✦ ') + this.theme.fg('customMessageLabel', this.theme.bold('Run recap'));
		...
		box.addChild(new Text(`${this.theme.fg('accent', this.theme.bold('Next:'))} ${this.theme.fg('customMessageText', this.data.next)}`, 0, 0));
		if (this.expanded) {
...
export async function openModelPicker(ctx: ExtensionCommandContext, _config: SummaryConfig) {
	const models = [...ctx.modelRegistry.getAvailable()].sort((a, b) => `${a.provider}/${a.id}`.localeCompare(`${b.provider}/${b.id}`));
...
export function openReasoningPicker(ctx: ExtensionCommandContext, model: Model<Api>, current: ReasoningLevel) {
