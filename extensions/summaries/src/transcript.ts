...
const SECRET_KEY_PATTERN = /(?:api[_-]?key|access[_-]?key|authorization|cookie|credential|password|passwd|private[_-]?key|secret|token)/i;
...
export function redactSecrets(text: string) {
	return text
		...
		.replace(/\b(sk-[A-Za-z0-9_-]{12,}|gh[pousr]_[A-Za-z0-9_]{12,}|eyJ[A-Za-z0-9_-]{12,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,})\b/g, '[REDACTED]')
		.replace(/(["']?(?:api[_-]?key|access[_-]?key|authorization|cookie|credential|password|passwd|private[_-]?key|secret|token)["']?\s*[:=]\s*)(["']?)[^\s,;}]+\2/gi, '$1[REDACTED]')
...
function sanitizeValue(value: unknown, key?: string, depth = 0): unknown {
	...
	if (typeof value === 'object' && value !== null) {
		return Object.fromEntries(Object.entries(value).map(([entryKey, entryValue]) => [entryKey, sanitizeValue(entryValue, entryKey, depth + 1)]));
	}
...
function serializeMessage(entry: Extract<SessionEntry, { type: 'message' }>) {
	...
	if (message.role === 'custom') {
		if (message.customType === 'summary-recap') {
			return '';
		}
...
export function serializeRunTranscript(entries: readonly SessionEntry[], maxBytes = TRANSCRIPT_MAX_BYTES) {
...
export function buildFallbackRecap(entries: readonly SessionEntry[]) {
	...
	for (const entry of entries) {
		...
		for (const block of entry.message.content) {
			...
			if (block.type === 'text' && block.text.trim()) {
				finalAssistantText = redactSecrets(
					block.text.trim(),
				);
			}
		}
	...
	const activity = tools.length > 0 ? ` The run used ${toolNames.length} tool call${toolNames.length === 1 ? '' : 's'} across ${tools.join(', ')}.` : '';

	const result = finalAssistantText ? ` ${capped(
		finalAssistantText.replace(/\s+/g, ' '),
		700,
